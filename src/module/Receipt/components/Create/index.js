...
import { getApiErrorMessage, getNumberFromStr, notify, vndCurrency } from '_utils/helpers'
...
import { mapReceiptPayload, splitReceipt, genNewMember } from '../../util'

export const CreateReceipt = props => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      const { data } = await ReceiptAPI.create(payload)

      notify({ message: data.message })
      setTimeout(
        () => history.push(
          generate([ 'receipt.edit', { _id: data._id } ])
        )
      )
    } catch (error) {
      notify({
  ...
  return (
    <Grid container justify="center" className="form-create-receipt">
